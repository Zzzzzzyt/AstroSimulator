	...
	public void run() {
		while (true) {
			...
			while (time < goal) {
				...
				if (now > last) {
					last = now;
					AstroSimulator.as.tmppos.get(0).add(sanae.get(0).getPos(now * 10000000));
					AstroSimulator.as.tmppos.get(1).add(sanae.get(1).getPos(now * 10000000));
					AstroSimulator.as.tmppos.get(2).add(sanae.get(2).getPos(now * 10000000));
				}
			}
			busy = false;
			if (sort) {
